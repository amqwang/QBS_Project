...
# print(class(final_comp))
# final_comp$A1BG <- as.numeric(final_comp$A1BG)
# 
# print(class(final_comp$A1BG[1]))
# print(is.numeric(final_comp))
# sapply(final_comp$A4GNT, is.numeric)

#Calculate correlations
corFinal <- cor(x= final_comp[15:30], method = 'spearman')

# Plot correlations
corrplot(corFinal,method = 'circle')



#generating variance values
geneVariance <- apply(corFinal, MARGIN = 1, FUN = var, )

#ordering rows of genes in order of decreasing variance
geneVariance <- corFinal[order(geneVariance,decreasing = T),]

# Log2-normalize data for plotting
log2GeneVariance <- log2(corFinal)

# Generate heatmap without clustering
pheatmap(log2GeneVariance[1:15,],
         cluster_rows = F,
         cluster_cols = F)
