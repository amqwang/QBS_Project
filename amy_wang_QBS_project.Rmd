...
```{r}
#merging the data into new dataframe called 'new_comp'
...
final_comp$age<- as.numeric(final_comp$age)

ggplot(final_comp, aes(x= age, y=AAGAB))+
  geom_point()+ #two age unknowns from the data
  labs(title = 'Age compared to distribution of gene AAGAB', x = 'Age (years)', y = 'Expression of AAGAB')+
...
```{r}
print(head(final_comp))
box_first <- dplyr::select(new_comp,AAGAB, sex, mechanical_ventilation) #selecting data columns for easier viewing
print(box_first)
...
ggplot(box_first, aes (sex, AAGAB, fill = mechanical_ventilation))+
  geom_boxplot()+
  theme_light()+
  labs(title = 'Distribution of AAGAB Based on Sex Coloured by Mechanical Ventilation', x = 'Sex', y = 'Expression of AAGAB')+
...
print(head(final_df)) #reformatting dataframe back to industry standard of having participant ID as the column names
```
